...
إنه أداة مفتوحة المصدر تتيح لك تحرير ملف قائمة القنوات الذي ينقله تلفزيونك إلى ذاكرة USB بصريًا في متصفحك باستخدام طريقة **السحب والإفلات**. لا يتطلب التثبيت، ويعمل مباشرةً عبر المتصفح أو عبر واجهة برمجة التطبيقات REST.
...
- **التحقق من التردد:** نشط فقط لبيانات قمر Türksat الصناعي.
- **حد حجم الملف:** لا يمكن أن يتجاوز حجم الملفات التي يتم تحميلها **2 ميغابايت**.
- **الجلسة المؤقتة:** لا يتم تخزين الملفات بشكل دائم، بل يتم حذفها تلقائيًا عند انتهاء الجلسة.
...
4. **التنزيل:** قم بتنزيل الملف الذي تم تنظيمه إلى جهاز الكمبيوتر الخاص بك.
...
```
*(يتم تنزيل الملف الثنائي/المضغوط المُعدّل)*

## 🔐 الخصوصية والأمان

- يوجد حد لحجم الملف يبلغ **2 ميغابايت**.
- **لا يتم تخزين الملفات بشكل دائم على الخادم.** تتم معالجة الملفات التي تم تحميلها في مساحة التخزين المؤقتة على الخادم طوال مدة جلسة التحرير، ولا يتم أرشفتها بشكل دائم، ويتم حذفها تلقائيًا بالكامل عند انتهاء مدة الجلسة (حوالي ساعة واحدة).
- لا يتم تسجيل أي حسابات أو عضويات أو بيانات قاعدة بيانات.
...
تتوفر الواجهة وأدلة الاستخدام **بـ 11 لغة**: التركية، الإنجليزية، الألمانية، الروسية، الإسبانية، الإيطالية، الفرنسية، العربية، الفارسية، الأذربيجانية، والبرتغالية.
